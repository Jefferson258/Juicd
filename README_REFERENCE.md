# Juicd — Extended reference

Step-by-step **out-of-code** tasks are condensed in **[SETUP.md](SETUP.md)**. This file keeps the longer narrative, migrations, and ops notes.

---

iOS app prototype — ranked sports picks, daily closest-pick tournament, friends (requests + MMR leaderboard), and profile badges. Uses a **local in-memory repository** with `UserDefaults` until you wire a backend.

**Ordered setup steps** → [SETUP.md](SETUP.md).
...
2. From the **repo root** (folder containing `Juicd.xcodeproj`):
...
- Short schema / ads notes: [`JUICD_SETUP.md`](JUICD_SETUP.md)
