...
///
/// Marked `nonisolated` so pure date math stays callable from any isolation domain (the app target uses default `MainActor` isolation).
enum JuicdSeason: Sendable {
    nonisolated private static func localCalendar() -> Calendar {
    ...
    nonisolated static func currentSeasonKey(at date: Date = .now) -> String {
    ...
    nonisolated static func shortLabel(for seasonKey: String) -> String {
    ...
    nonisolated static func range(for seasonKey: String) -> (start: Date, end: Date)? {
        let parts = seasonKey.split(separator: "-")
        guard parts.count == 2,
    ...
    nonisolated static func contains(_ date: Date, seasonKey: String) -> Bool {
    ...
    nonisolated static func nextSeasonStart(from date: Date = .now) -> Date? {
